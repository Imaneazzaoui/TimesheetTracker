  ...
  return (
    <div className="h-screen w-full fixed left-0 top-0 flex justify-end items-center" style={{ backgroundColor: "rgba(0, 0, 0, 0.5)" }}>
      <form className="bg-white rounded-lg shadow-2xl w-1/4 flex flex-col items-start" style={{ height: "100vh" }}>
        <header className="bg-indigo-800 pb-2 flex justify-between w-full">
          <div></div>
          <h2 className="text" style={{ fontSize: '15px', color: 'white' }}>Ajouter une feuille de temps</h2>
          <button onClick={() => setShowEventModal(false)}>
            <span className="material-icons-outlined  pt-3 text-white">close</span>
          </button>
          <button
            onClick={() => {
              dispatchCalEvent({
            ...
            className='material-icons-outlined pr-4 pt-2 text-rose-600	'
            id="deletebutton"
          >
            ...
            <div className="relative w-full">
              ...
              {showAdditionalInputs && (
                <div className="grid gap-4 grid-cols-2">
                  ...
                  <div className='pt-4'>
                    {/* Champ d'entrée supplémentaire pour la consommation */}
                    <input
                      type="number"
                      step="0.25"
        ...
        <div className="w-full mt-4 flex justify-center">
            <p className="text-lg font-semibold">Total Consommation: {totalConsumption.toFixed(2)}</p>
        </div>*/}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <button className='bg-blue-900 px-5 py-4 ml-2 my-3 rounded text-white text-sm w-full' type="submit"
                onClick={handleSubmit} id='savebutton'>Enregistrer</button>
            </div>
            <div>
